...
NWALL=0 #$(grep nsteps_wall procedure.lmp|head -n1 | grep equal |awk '{print $4}')
NCOEX=$(grep run Restart.lmp |awk '{print $2}')
let NUM_STOP=$NWALL+$NCOEX
if [ ${NUM_DONE} -ge ${NUM_STOP} ]; then
  exit
fi

sbatch --job-name="$SBATCH_JOB_NAME"  /projects/bblh/sigbjobo/PhaseDiagram/III/job.sh
